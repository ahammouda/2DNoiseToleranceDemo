...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\textwidth]{classic_division.eps}
...
The solution of partial differential equations (PDEs) are classic problems for parallel
computers and use
...
\begin{algorithm}[h]
  \setcounter{AlgoLine}{0}
  \For{$\textsf{tile} \in \text{tiles in region }a_xa_y$}{
    \For{$x \la 1$ \KwTo $H(\textsf{tile})$}{
      \For{$y \la 1$ \KwTo $\text{\bf min}( V(\textsf{tile}),V(\textsf{tile}) - x + h(\textsf{tile}) )$}{
        $\textsf{update}(x,y).$
      }
    }
  }
...
To actually employ rotations properly, we'll need to know what rotations actually need to be performed by
...
\begin{algorithm}[h]
  ...
  \setcounter{AlgoLine}{0}
  \Procedure{$\mathcal{G}^M(a_xa_y \in \mathcal{R}, \Theta)$}{
    \When{$a_xa_y = 00$}{
